...
async function simulateProposed(
	...
	if (!proposalCreatedEvent)
		throw new Error(
			`Proposal creation log for #${proposalId} not found in governor logs`,
		);
	const {
		targets,
		signatures: sigs,
...
async function sendSimulation(
	...
	try {
		...
		const sim = <TenderlySimulation>(
			await fetchUrl(TENDERLY_SIM_URL, fetchOptions)
		);
